...
// Initialize endpoint (replaces global side-effects)
app.post('/make-server-52d68140/system/init', async (c) => {
  try {
    const authHeader = c.req.header('Authorization');
    // Basic protection or just allow admins
    const admin = await verifyAdmin(c.req.raw);
    if (!admin && authHeader) return c.json({ error: 'Unauthorized' }, 401);

    await initializeDefaultUsers();
    await initializeGalleryBucket();
    return c.json({ success: true, message: 'System initialized' });
  } catch (error) {
    return c.json({ error: 'Init failed' }, 500);
  }
});
...
app.get('/make-server-52d68140/products/lightweight', async (c) => {
  try {
    ...
    // Filter out corrupted data (non-objects or missing fields)
    // A valid product must be an object and have at least a name
    const products = all.filter((p: any) => p && typeof p === 'object' && p.name);
    ...
    const lightweight = products.map((p: any) => ({
      ...
      secondaryImage: p.secondaryImage || (p.images && p.images[1]) || null,
      images: p.images || [],
      category: p.category || 'Uncategorized',
      bestSeller: !!p.bestSeller,
      newArrival: !!p.newArrival,
      budgetFind: !!p.budgetFind,
      premiumCollection: !!p.premiumCollection
    }));
    return c.json({ products: lightweight });
  } catch (error) {
    console.log('Get lightweight products error:', error);
  ...
  try {
    ...
    if (nameQuery) {
      const target = products.find(p => {
        ...
        return slug === nameQuery.toLowerCase();
      });
      if (target) return c.json({ product: target });
...
app.get('/make-server-52d68140/best-sellers', async (c) => {
  try {
    const allItems = await kv.getByPrefix('product:') || [];
    const all = allItems.filter((p: any) => p && typeof p === 'object' && p.name);
    const flagged = all.filter((p: any) => !!p.bestSeller);
    const pick = (flagged.length > 0 ? flagged : all).slice(0, 20);
    const lightweight = pick.map((p: any) => ({
      id: p.id || 'missing-id',
      name: p.name || 'Untitled Product',
      price: Number(p.price || 0),
      image: p.image || (p.images && p.images[0]) || '',
...
app.get('/make-server-52d68140/home-section/:section', async (c) => {
  try {
    ...
    const lightweight = filtered.map((p: any) => ({
      id: p.id || 'missing-id',
      name: p.name || 'Untitled Product',
      price: Number(p.price || 0),
      image: p.image || (p.images && p.images[0]) || '',
...
async function getCloudinarySignature(params: Record<string, any>) {
...
// Product Image Upload - Now using Cloudinary (Legacy route, might be removable if frontend updated)
app.post('/make-server-52d68140/products/upload', async (c) => {
  try {
    ...
    const { data, error } = await supabase.storage.from('make-52d68140-gallery').upload(fileName, file);

    if (error) throw error;
    const { data: { publicUrl } } = supabase.storage.from('make-52d68140-gallery').getPublicUrl(fileName);
...
app.get('/make-server-52d68140/videos', async (c) => {
  try {
    ...
    const sorted = (videos || []).sort((a: any, b: any) => (a.order || 0) - (b.order || 0));
...
app.get('/make-server-52d68140/videos/:id/likes', async (c) => {
  try {
    const id = c.req.param('id');
    const likeData = await kv.get(`video_likes:${id}`) || { users: [] };
    return c.json({ count: (likeData.users || []).length });
  } catch (error) {
    console.log('Get video likes error:', error);
...
app.post('/make-server-52d68140/videos/thumbnail/upload', async (c) => {
  try {
    ...
    const publicId = `videos/thumbs/${Date.now()}-${fileName.replace(/\.[^/.]+$/, '')}`;
    const timestamp = Math.floor(Date.now() / 1000).toString();
    const signature = await getCloudinarySignature({ public_id: publicId, timestamp });
    ...
    const resp = await fetch(`https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD_NAME}/image/upload`, { method: 'POST', body: form });
    const cd = await resp.json();
    if (!resp.ok) return c.json({ error: cd.error?.message || 'Upload failed' }, 400);
    return c.json({ success: true, url: cd.secure_url });
  } catch (error) {
...
app.post('/make-server-52d68140/videos/upload', async (c) => {
  try {
    ...
    let body: any = {};
    try { body = JSON.parse(raw); } catch { }
    const { video, fileName } = body;
    if (!video || !fileName) return c.json({ error: 'Video and fileName are required' }, 400);

    const publicId = `videos/${Date.now()}-${fileName.replace(/\.[^/.]+$/, '')}`;
    const timestamp = Math.floor(Date.now() / 1000).toString();
    const signature = await getCloudinarySignature({ public_id: publicId, timestamp });
    ...
    const resp = await fetch(`https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD_NAME}/video/upload`, { method: 'POST', body: form });
    const cd = await resp.json();
    if (!resp.ok) return c.json({ error: cd.error?.message || 'Upload failed' }, 400);
    return c.json({ success: true, url: cd.secure_url });
  } catch (error) {
...
// Offers Routes (Postgres)
app.get('/make-server-52d68140/offers', async (c) => {
  try {
    const admin = await verifyAdmin(c.req.raw);
    if (!admin) return c.json({ error: 'Admin access required' }, 403);
    ...
    return c.json({ error: 'Failed to fetch offers', details: (error as Error).message }, 500);
  }
});

app.get('/make-server-52d68140/offers/active', async (c) => {
  try {
    ...
    console.log('Get active offer error:', error);
    return c.json({ error: 'Failed to fetch active offer' }, 500);
  }
});

app.post('/make-server-52d68140/offers/verify-coupon', async (c) => {
  try {
    ...
    if (error || !offer) {
      return c.json({ error: 'Invalid or expired coupon' }, 404);
    }

    if (offer.min_order_amount && cartTotal < offer.min_order_amount) {
      return c.json({ error: `Minimum order of ₹${offer.min_order_amount} required` }, 400);
    }

    return c.json({ success: true, offer });
  } catch (error) {
    console.log('Verify coupon error:', error);
    return c.json({ error: 'Failed to verify coupon' }, 500);
  }
});

app.post('/make-server-52d68140/offers', async (c) => {
  try {
    const admin = await verifyAdmin(c.req.raw);
    if (!admin) return c.json({ error: 'Admin access required' }, 403);
    ...
    return c.json({ error: 'Failed to create offer', details: (error as Error).message }, 500);
  }
});

app.put('/make-server-52d68140/offers/:id', async (c) => {
  try {
    const admin = await verifyAdmin(c.req.raw);
    if (!admin) return c.json({ error: 'Admin access required' }, 403);
    ...
    return c.json({ error: 'Failed to update offer', details: (error as Error).message }, 500);
  }
});

app.delete('/make-server-52d68140/offers/:id', async (c) => {
  try {
    const admin = await verifyAdmin(c.req.raw);
    if (!admin) return c.json({ error: 'Admin access required' }, 403);
    ...
    return c.json({ error: 'Failed to delete offer', details: (error as Error).message }, 500);
  }
});

// Rating & Review Routes
app.get('/make-server-52d68140/reviews/:productId', async (c) => {
  try {
    const productId = c.req.param('productId');
...
app.post('/make-server-52d68140/reviews', async (c) => {
  try {
    const user = await verifyAuth(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);

    const body = await c.req.json();
    const { product_id, rating, comment, image_url, video_url } = body;
...
app.post('/make-server-52d68140/reviews/:id/vote', async (c) => {
  try {
    const user = await verifyAuth(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    ...
    const { data: existing } = await supabase.from('reviews').select(field).eq('id', id).single();
    ...
    console.log('Vote review error:', error);
    return c.json({ error: 'Failed to record vote' }, 500);
  }
});

app.delete('/make-server-52d68140/reviews/:id', async (c) => {
  try {
    const user = await verifyAuth(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
...
// Cloudinary Signature Endpoint - for secure uploads from frontend
app.post('/make-server-52d68140/cloudinary-signature', async (c) => {
  try {
    ...
    const body = await c.req.json();
    const { public_id, timestamp, folder, format } = body;
    ...
    // Build params to sign - must include ALL params sent to Cloudinary
    // except file, api_key, and signature.
    const paramsToSign: Record<string, string> = { ...body };

    // Remove fields that should NOT be signed or are handled internally
    ...
    return c.json({
      signature,
      api_key: Deno.env.get('CLOUDINARY_API_KEY') || '393724131116165',
      cloud_name: Deno.env.get('CLOUDINARY_CLOUD_NAME') || 'dxpabpzkf'
    });
  } catch (error) {
...
// Hero Images Routes - Stores Cloudinary image URLs
app.get('/make-server-52d68140/hero-images', async (c) => {
  try {
    const heroImages = await kv.getByPrefix('heroImage:');
    const sorted = (heroImages || [])
      .filter((img: any) => img.active !== false)
      .sort((a: any, b: any) => (a.order || 0) - (b.order || 0));
    return c.json({ heroImages: sorted });
  } catch (error) {
    console.log('Get hero images error:', error);
    return c.json({ error: 'Failed to fetch hero images' }, 500);
  }
});

app.get('/make-server-52d68140/hero-images/all', async (c) => {
  try {
    const admin = await verifyAdmin(c.req.raw);
    if (!admin) return c.json({ error: 'Admin access required' }, 403);

    const heroImages = await kv.getByPrefix('heroImage:');
    const sorted = (heroImages || []).sort((a: any, b: any) => (a.order || 0) - (b.order || 0));
    return c.json({ heroImages: sorted });
  } catch (error) {
    console.log('Get all hero images error:', error);
    return c.json({ error: 'Failed to fetch hero images' }, 500);
  }
});

app.post('/make-server-52d68140/hero-images', async (c) => {
  try {
    const user = await verifyAuth(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    const profile = await kv.get(`user:${user.id}`);
    ...
    const body = await c.req.json();
    const { title, imageUrl, imageUrlMobile, order, active } = body;
    ...
    await kv.set(`heroImage:${id}`, heroImage);
    return c.json({ success: true, heroImage });
  } catch (error) {
    console.log('Create hero image error:', error);
    return c.json({ error: 'Failed to create hero image' }, 500);
  }
});

app.put('/make-server-52d68140/hero-images/:id', async (c) => {
  try {
    const admin = await verifyAdmin(c.req.raw);
    if (!admin) return c.json({ error: 'Admin access required' }, 403);

    const id = c.req.param('id');
    const existing = await kv.get(`heroImage:${id}`);
    if (!existing) return c.json({ error: 'Hero image not found' }, 404);

    const updates = await c.req.json();
    const updated = { ...existing, ...updates, updatedAt: new Date().toISOString() };
    await kv.set(`heroImage:${id}`, updated);
    return c.json({ success: true, heroImage: updated });
  } catch (error) {
    console.log('Update hero image error:', error);
    return c.json({ error: 'Failed to update hero image' }, 500);
  }
});

app.delete('/make-server-52d68140/hero-images/:id', async (c) => {
  try {
    const admin = await verifyAdmin(c.req.raw);
    if (!admin) return c.json({ error: 'Admin access required' }, 403);

    const id = c.req.param('id');
    await kv.del(`heroImage:${id}`);
    return c.json({ success: true });
  } catch (error) {
    console.log('Delete hero image error:', error);
